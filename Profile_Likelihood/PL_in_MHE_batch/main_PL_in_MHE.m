%% Profile Likelihood in moving time horizon
...
% load data from moving horizon estimation
load('P_save.mat')
...
%% #############################################################################################################
        ...
        parfor i = 1:N_multistart
            ...
            [para_opt,Obj_opt_mst,flag]  = fmincon(@(decision_var)obj_MHE(decision_var,x_tH_0,Para_prev,y_tH,tH,P,p),decision_var,[],[],[],[],log10(lb_para),log10(ub_para),[],p.opt_fmincon);
            ...
            parfor k = 1:size(var_para,2) % vary one parameter at a time
                ...
                P_var_sol = [];
                       ...
                       while solver_success ~= 1  
                            ...
                            % call fmincon
                           [P_var_sol,obj,flag]  = fmincon(@(decision_var)obj_MHE_PL(decision_var,para_states_full_pl,x_tH_0,Para_prev,y_tH,tH,P,idx_free,idx_full,p),decision_var,[],[],[],[],log10(lbw),log10(ubw),[],p.opt_fmincon);
